...
function authorized(req){let supplied=(req.headers.authorization||'').replace(/^Bearer\s+/,'');let expected=process.env.GENERATION_ACCESS_KEY||'';if(!supplied||!expected)return false;let a=Buffer.from(supplied),b=Buffer.from(expected);return a.length===b.length&&timingSafeEqual(a,b)}
